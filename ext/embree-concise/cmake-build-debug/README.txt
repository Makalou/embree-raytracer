...
-   `RTC_ERROR_CANCELLED`: The operation got canceled by a memory
    monitor callback or rendering_progress monitor callback function.
...
#### NAME {#name}
    ...
    rtcSetSceneProgressMonitorFunction - registers a callback
      to track build rendering_progress
...
#### SYNOPSIS {#synopsis}
    ...
    void rtcSetSceneProgressMonitorFunction(
      RTCScene scene,
      RTCProgressMonitorFunction rendering_progress,
      void* userPtr
    );

#### DESCRIPTION {#description}

Embree supports a rendering_progress monitor callback mechanism that can be used
to report rendering_progress of hierarchy build operations and to cancel build
operations.

The `rtcSetSceneProgressMonitorFunction` registers a rendering_progress monitor
callback function (`rendering_progress` argument) with payload (`userPtr`
...
in the range $[0, 1]$ which estimates the rendering_progress of the operation
...
rendering_progress (`buildProgress` member) is optional and may also be `NULL`.
